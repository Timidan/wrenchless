...
import { useTravelSafeV2 } from "../useTravelSafe";

/**
 * The Travel Safe, on the one device that holds it — the existing v2 lock and
 * return flow, held exactly as it shipped.
 *
 * v2 tickets keep this surface and `useTravelSafeV2` for as long as they
 * exist: the schema stores a live recovery phrase, and nothing about how that
 * ticket reads or moves changes because a newer Trip Allowance exists. `
 * SafeSurface` is the only thing that decides which of the two a browser
 * sees; once it has decided on v2 nothing here needs to know v3 exists.
 *
...
export function SafeSurfaceV2(): JSX.Element {
  const { model, actions } = useTravelSafeV2();
