...
export async function retrieveGuardianHeartbeats(input: {
  ...
  for (const envelope of contents.envelopes) {
    try {
      opened.push(await openHeartbeat(envelope, input.guardianPrivateKey));
    } catch {
      // The mailbox is an untrusted bearer-write store. One unreadable item
      // must not hide a valid signal that arrived beside it.
    }
  }
  return opened;
}
